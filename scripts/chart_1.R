...
chart_1 <- function (netflix_data, disney_data, amazon_data){
  ...
  netflix_genres <- netflix_genres[-c(12),]
  netflix_total <- netflix_genres %>%
    summarize(sum_shows = sum(n)) %>%
    pull(sum_shows)
  netflix_genres <- netflix_genres %>%
    mutate(percents = round((n/netflix_total) * 100, 2)) %>%
    top_n(5)
  ...
  disney_total <- disney_genres %>%
    ...
    pull(sum_shows)
  disney_genres <- disney_genres %>%
    mutate(percents = round((n/disney_total) * 100, 2)) %>%
    top_n(5)
  ...
  amazon_total <- amazon_genres %>%
    ...
    pull(sum_shows)
  amazon_genres <- amazon_genres %>%
    mutate(percents = round((n/amazon_total) * 100, 2)) %>%
    top_n(5)
  ...
  #make bar chart
  ggplot(data = joined, aes()) +
    geom_col(mapping = aes(x = streaming_service, y = percents, fill = genre),
             position = "dodge") +
